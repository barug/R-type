  ...
  _readFd()
{
  ...
  if ( (_fd = socket(AF_INET, SOCK_DGRAM, 0)) == -1 )
    throw std::runtime_error(std::string("Error ! Cannot creat the socket."));
  ...
  if(!getsockname(_fd, (struct sockaddr *)&s_in, &addressLength))
    {
      _port = ntohs(s_in.sin_port);
      std::cout << "Vlaaaa lpooooort " << s_in.sin_port << std::endl;
    }
}
